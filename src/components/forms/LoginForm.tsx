...
export default function LoginForm() {
 ...
 const handleSuccess = (role: 'farmer' | 'buyer') => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      form.id_number === PREDEFINED_CREDENTIALS.farmer.id_number &&
    ...
    if (
      form.id_number === PREDEFINED_CREDENTIALS.buyer.id_number &&
    ...
    try {
      console.log('Attempting API login for:', form.id_number); // Add log for clarity
...
+     login()
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 animate-slideIn px-1">
      <div>
        <label htmlFor="email" className="block text-sm font-medium text-gray-700">National ID Number</label>
